...
`SteamworksUgcState` keeps the latest query result variants and also caches bounded query request/result snapshots by plugin `request_id`, plus item details, item state, download progress, install info snapshots, and download completion callbacks by `PublishedFileId`. Systems can query these with `query_request`, `query_result`, `query_total_result`, `query_ids_result`, `item_details`, `item_detail`, `item_creator_app_id`, `item_consumer_app_id`, `item_title`, `item_description`, `item_tags`, `item_preview_url`, `item_content_descriptors`, `item_statistics`, `item_statistic`, `item_metadata`, `item_children`, `item_key_value_tags`, `item_key_value_tag`, `item_state`, `item_download_info`, `item_install_info`, `download_item_results`, `download_item_result`, and `download_item_failed` without retaining message history. Deleting an item clears its item-level cached snapshots.
