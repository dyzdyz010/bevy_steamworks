...
`SteamworksUgcState` keeps the latest query result variants and also caches bounded item details, item state, download progress, and install info snapshots by `PublishedFileId`. Systems can query these with `item_details`, `item_detail`, `item_state`, `item_download_info`, and `item_install_info` without retaining message history. Deleting an item clears its item-level cached snapshots.
