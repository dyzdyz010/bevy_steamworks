...
Use `poll_all_listen_socket_events(...)` and `poll_all_connection_events(...)` for frame-level engine systems that want to service every socket or connection owned by the plugin without keeping their own ID iteration list.
