...
Use `poll_all_listen_socket_events(...)`, `poll_all_connection_events(...)`, `receive_all_messages(...)`, `receive_all_poll_group_messages(...)`, and `flush_all_messages()` for frame-level engine systems that want to service every socket, connection, or poll group owned by the plugin without keeping their own ID iteration list.
