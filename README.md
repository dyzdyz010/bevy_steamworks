...
`SteamworksRemotePlayState` caches the latest bulk session list, bounded ID-based known session snapshots, bounded callback-observed connected session IDs, the latest submitted Remote Play Together invite, and successful invite count. Systems can query specific ID-based snapshots with `known_session`, group them with `known_sessions_for_user`, read the latest user session with `latest_known_session_for_user`, inspect fields through `session_user`, `session_client_name`, `session_client_form_factor`, and `session_client_resolution`, check callback-observed connectivity with `is_session_observed_connected`, and inspect the last invite through `last_submitted_invite`.
