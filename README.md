...
fn main() {
    if SteamworksPlugins::restart_app_if_necessary(480) {
        return;
    }
...
`480` is Valve's Spacewar sample app id. Real games should use the app id assigned by Valve. `SteamworksPlugins::restart_app_if_necessary(...)` wraps Steam's launch check and should run before Steamworks initialization; return from `main` when it asks Steam to relaunch the app. The same helper is also available on `SteamworksPlugin` when you build the core lifecycle manually.
...
`SteamworksServerPlugins` initializes the upstream `steamworks::Server`, inserts `SteamworksServer` as a Bevy resource, registers `SteamworksServerCommand` / `SteamworksServerResult`, pumps Steam Game Server callbacks into the shared `SteamworksEvent` message stream, and installs the server-compatible high-level feature plugins for legacy networking, networking messages, networking sockets, read-only utils, and UGC game-server Workshop initialization. Dedicated server initialization is separate from `SteamworksPlugin`; use one lifecycle for the process unless you have a specific reason to initialize both.

Use `SteamworksServerFeaturePlugins` with a separately configured `SteamworksServerPlugin` when a wrapper needs to own the lifecycle plugin but still wants the default server-side command layers.

Server wrappers and diagnostics can inspect dedicated server lifecycle settings without initializing Steam by reading `init_mode()`, `failure_policy_setting()`, and `runs_callbacks()` from either `SteamworksServerPlugin` or the full `SteamworksServerPlugins` group. When initialization is allowed to fail, the plugin inserts `SteamworksServerUnavailable`; the resource exposes `is_manual_server_missing()`, `is_invalid_string()`, `is_init_failed()`, `invalid_string_field()`, `init_config()`, and `init_error()` helpers for structured diagnostics.
...
fn request_social(mut friends: MessageWriter<SteamworksFriendsCommand>) {
    ...
    friends.write(SteamworksFriendsCommand::activate_game_overlay_dialog(
...
Commands validate strings before calling upstream `steamworks` methods, so interior NUL bytes are reported as `SteamworksFriendsError::InvalidString` instead of panicking inside a Steam C string conversion. Built-in overlay targets are available through `SteamworksGameOverlayDialog` and `SteamworksUserOverlayDialog`, while raw string overlay commands remain available for compatibility. Friend list results use snapshot types such as `SteamworksFriendInfo`, which are safe to store in ECS resources or messages. `SteamworksFriendsState` keeps the latest friend list plus bounded per-user, Rich Presence, relationship, and avatar caches; systems can query `friend_count`, `known_friend_count`, `online_friends`, `friends_in_game`, `friends_playing_game`, `friends_in_lobby`, `friend`, `has_known_friend`, `friend_name`, `friend_nickname`, `friend_state`, `friend_game`, `friend_is_in_game`, `friend_is_in_lobby`, `coplay_friend_count`, `coplay_friend`, `coplay_app_id`, `coplay_time`, `friend_rich_presence`, `friend_avatar`, `friend_avatar_dimensions`, `friend_avatar_rgba`, `has_friend`, and the latest overlay/invite actions without retaining result history. Rich Presence and avatar accessors use an outer `Option` for unread data and an inner `Option` for a completed read where Steam had no value or image available yet. Overlay activation, persona state changes, lobby join requests, and Rich Presence join requests are still available through `SteamworksEvent`, and are also mirrored as `SteamworksFriendsResult` messages for module-local systems.
...
`SteamworksMatchmakingState` caches bounded lobby snapshots for submitted and completed request contexts, lobby list results, created/joined/left lobbies, metadata reads and mutations, member reads, joinability changes, chat sends and entry reads, lobby game-server reads, and lobby callbacks. Systems can query lobby-list/create/join results by `request_id`, and lobby metadata/member/chat/game-server snapshots by lobby/key/index instead of retaining every result message. Convenience accessors such as `last_lobby_list_count`, `joined_lobby_count`, `last_joined_lobby_id`, `is_lobby_joined`, `lobby_data_count_value`, `lobby_data`, `has_lobby_data`, `all_lobby_data_value`, `lobby_member_data`, `has_lobby_member_data`, `lobby_member_ids`, `has_lobby_member`, `lobby_owner_id`, `lobby_member_count_value`, `lobby_member_limit_value`, `lobby_joinable`, `lobby_chat_entry_data`, `lobby_chat_entry_len`, `last_lobby_chat_entry_data`, `has_lobby_game_server`, `lobby_game_server_address`, and `lobby_game_server_steam_id` expose common lobby UI checks without unpacking snapshot structs. Its joined lobby list tracks joins observed by this plugin and explicit leave commands; it is not a strong-consistency source for remote kicks, disconnects, or lobby shutdowns.
...
`SteamworksMatchmakingServersState` caches bounded server-browser snapshots: active request count, submitted server-list requests, direct query contexts and results, latest submitted/released server-list request, latest refresh and single-server refresh submissions, latest count/refreshing reads, server response/failure/details contexts, server snapshots, and refresh completion counters. Systems can query `server_list_request`, `server_list_count`, `server_list_refreshing`, `server`, `server_query`, `server_query_target`, `server_query_kind`, `server_ping`, `server_ping_server`, `server_ping_latency`, `server_ping_server_name`, `server_ping_map`, `server_ping_player_count`, `server_ping_max_players`, `server_player_details`, `server_players`, `server_player_count`, `server_player`, `server_has_player`, `server_player_score`, `server_player_time_played`, `server_rules`, `server_rule_entries`, `server_rule_count`, `server_rule`, and `server_has_rule` by request/query ID instead of retaining every result message. Releasing a server-list request clears cached list data for that request while preserving direct-query snapshots. It keeps counters and bounded snapshots instead of retaining unbounded server or callback history.
...
`SteamworksInputState` caches bounded known controller snapshots, named action set/digital/analog handles, action set activations by controller, digital/analog action data by controller and action, digital/analog origin snapshots by controller/action-set/action, action origin presentation data observed from origin reads, motion data by controller, binding panel controller, and successful `RunFrame` count. Systems can query the cached state with accessors such as `controller_count`, `controller`, `has_controller`, `controller_input_type`, `action_set_count`, `action_set_handle`, `has_action_set_handle`, `digital_action_count`, `digital_action_handle`, `has_digital_action_handle`, `analog_action_count`, `analog_action_handle`, `has_analog_action_handle`, `action_set_activation`, `active_action_set`, `digital_action_data`, `digital_action_pressed`, `digital_action_active`, `analog_action_data`, `analog_action_mode`, `analog_action_vector`, `analog_action_active`, `digital_action_origins`, `digital_action_origin_count`, `analog_action_origins`, `analog_action_origin_count`, `action_origin_info`, `action_origin_glyph_path`, `action_origin_name`, `motion`, `has_motion`, `motion_rotation_quaternion`, `motion_position_acceleration`, `motion_rotation_velocity`, `binding_panel_was_shown`, and the `last_*` accessors. `Init`, `Shutdown`, and `SetActionManifestFilePath` clear stale action data so reads from a previous manifest are not silently reused.
...
`SteamworksNetworkingState` caches the latest accepted/closed session remote, bounded session state snapshots by remote, bounded packet availability snapshots by channel, latest sent packet summary, bounded received packet snapshots, empty-read count/channel, and bounded session request/failure callback snapshots. Systems can query `session_state`, `p2p_session_state`, `session_active`, `session_connecting`, `session_using_relay`, `session_error`, `session_bytes_queued_for_send`, `session_packets_queued_for_send`, `session_remote_ip`, `session_remote_port`, `packet_availability`, `packet_available_bytes`, `packet_available`, `last_sent_packet_remote`, `last_sent_packet_channel`, `last_sent_packet_bytes`, `received_packets_from`, `received_packet_count_from`, `received_packets_on_channel`, `received_packet_count_on_channel`, `last_packet_from`, `last_packet_bytes_from`, `last_packet_on_channel`, `last_packet_bytes_on_channel`, `last_packet_data`, `has_session_request`, `session_connect_failure`, `has_session_connect_failure`, and `session_connect_failure_error` without retaining every result message. It keeps counters and bounded caches instead of unbounded packet or callback history, so gameplay systems can poll state resources without growing memory over time.
...
Session requests are accepted by default because the upstream safe API only allows accepting or rejecting while the Steam callback is running; it cannot defer the decision to a later ECS frame. Use `SteamworksNetworkingMessagesPlugin::new().auto_accept_session_requests(false)` or `SteamworksNetworkingMessagesCommand::set_auto_accept_session_requests(false)` to reject future incoming requests by default, then add peer-specific overrides with `accept_session_requests_from(peer)`, `reject_session_requests_from(peer)`, or `clear_session_request_decision(peer)`. Engine layers can inspect the plugin default before build with `auto_accepts_session_requests()`. Policy commands are pre-read before `SteamworksSystem::RunCallbacks`, then processed normally later so you still receive result messages. Every session request and session failure is emitted as a `SteamworksNetworkingMessagesResult`.

Received messages are copied into `SteamworksNetworkingMessage { data: Vec<u8>, .. }`, so they can safely be stored in Bevy resources after Steam releases the original message handle. `SteamworksNetworkingMessagesState` caches the latest receive batch, bounded recent received messages across batches, bounded session request/failure callback snapshots, peer-specific session request decisions, and accepted/rejected request counts; systems can query `received_message_count`, `recent_received_message_count`, `last_received_message`, `last_received_message_peer`, `last_received_message_channel`, `last_received_message_bytes`, `last_received_message_data`, `received_messages_on_channel`, `received_message_count_on_channel`, `recent_received_messages_on_channel`, `last_recent_received_message_on_channel`, `received_messages_from_peer`, `received_message_count_from_peer`, `recent_received_messages_from_peer`, `last_recent_received_message_from_peer`, `last_recent_received_message_bytes_from_peer`, `last_connection_state`, `last_connection_remote`, `last_connection_user_data`, `last_connection_end_reason`, `last_connection_ping`, `last_connection_quality`, `session_requests`, `session_request`, `has_session_request`, `session_request_accepted`, `session_request_decisions`, `session_request_decision`, `session_request_decision_accepts`, `session_accept_count`, `session_reject_count`, `session_failures`, `session_failure`, `has_session_failure`, `session_failure_state`, and `session_failure_end_reason` without retaining every result message. Channel values are validated before the upstream API call to avoid signed integer wrapping, and one receive command is capped by `STEAMWORKS_NETWORKING_MESSAGES_MAX_BATCH_SIZE` to prevent unbounded frame-loop allocation.
...
`SteamworksNetworkingSocketsState` caches bounded snapshots for the latest created and closed handles, polled event batches, connection info, realtime status, sent message context, batch send outcomes, received message batches, flushes, poll-group assignments, lane configuration, user-data reads or updates, and connection debug-name updates. Systems can query handle-keyed state with `listen_socket_event_batch`, `listen_socket_event_count`, `connection_event_batch`, `connection_event_count`, `connection_event_removed`, `connection_info`, `connection_info_count`, `connection_state`, `connection_remote`, `connection_end_reason`, `connection_user_data`, `connection_name`, `realtime_status`, `connection_ping`, `connection_quality`, `connection_send_rate_bytes_per_sec`, `connection_pending_unreliable`, `connection_pending_reliable`, `connection_lane_count`, `last_sent_message_number`, `last_sent_message_bytes`, `last_sent_message_for_connection`, `last_sent_message_number_for_connection`, `last_sent_message_succeeded_for_connection`, `sent_message_count_for_connection`, `received_messages_for_connection`, `received_message_count_for_connection`, `last_received_message_for_connection`, `last_received_message_bytes_for_connection`, `last_received_message_data_for_connection`, `poll_group_messages`, `poll_group_message_count`, `last_poll_group_message`, `last_poll_group_message_bytes`, and `last_poll_group_message_data` without retaining every result message. Message and command `Debug` output reports payload lengths instead of raw bytes so tracing does not dump packet contents.
...
`SteamworksScreenshotsState` caches the latest hook state, successful trigger count, screenshot request callback count, bounded accepted library submission metadata, bounded screenshot-ready callback snapshots, and the latest screenshot ready callback. Systems can keep using `added_screenshots` for handle-only compatibility, query `submitted_screenshots`, `submitted_screenshot`, `submitted_screenshot_by_filename`, `submitted_screenshot_dimensions`, `submitted_screenshot_thumbnail`, and `last_submitted_screenshot` when they need submitted path and dimensions, or query `screenshot_ready_events`, `screenshot_ready`, `screenshot_ready_count`, `screenshot_ready_success_count`, `screenshot_ready_error_count`, `last_screenshot_ready`, and `last_screenshot_ready_error` when they need save confirmations without retaining every result message.
...
`SteamworksRemoteStorageState` caches the latest Cloud info, file list, bounded full file-info snapshots, individual file existence/persistence/timestamp/sync-platform reads, file contents, writes, and shared-file results. Systems can query `cloud_enabled_for_app`, `cloud_enabled_for_account`, `cloud_available`, `file_names`, `file_summary`, `file_size`, `file_info`, `file_exists`, `file_persisted`, `file_timestamp`, and `file_sync_platforms` by name instead of retaining every result message. `SteamworksRemoteStorageCommand::read_file(...)`, `write_file(...)`, and `share_file(...)` emit `FileReadRequested`, `FileWriteRequested`, or `FileShareRequested` immediately with plugin-assigned `request_id` values; the state also keeps bounded request/result lookups through `file_read_request`, `file_contents_by_request`, `file_contents_by_name`, `file_data`, `file_data_by_request`, `file_read_bytes`, `file_read_bytes_by_request`, `file_write_request`, `file_write`, `file_write_bytes`, `file_write_by_name`, `file_write_bytes_by_name`, `file_share_request`, `shared_file`, `shared_file_by_name`, `shared_file_handle`, `shared_file_handle_raw`, and `has_shared_file`. File payload reads and writes run on background workers and later emit `FileRead`, `FileWritten`, or a structured async error through `SteamworksRemoteStorageResult`; async read errors include the request ID for correlation. `FileWritten` means the upstream writer accepted the bytes and the stream close was issued; because write may invalidate old metadata, cached full file info for that name is cleared until it is read again. File payload `Debug` output reports byte lengths instead of raw bytes. File names are validated before calling upstream `steamworks`, so interior NUL bytes become `SteamworksRemoteStorageError::InvalidString` instead of panicking in a C string conversion.
...
`SteamworksUgcState` keeps the latest query result variants and also caches bounded query request/result snapshots by plugin `request_id`, plus item details, item state, download progress, install info snapshots, and download completion callbacks by `PublishedFileId`. Systems can query these with `query_request`, `query_result`, `query_result_items`, `query_result_item_count`, `query_result_total_count`, `query_result_was_cached`, `last_query_item_count`, `last_query_total_count`, `last_query_was_cached`, `query_total_result`, `query_total_count`, `query_ids_result`, `query_ids_items`, `query_ids_item_count`, `last_query_ids_count`, `subscribed_item_count`, `is_item_subscribed`, `item_details`, `item_detail`, `item_creator_app_id`, `item_consumer_app_id`, `item_title`, `item_description`, `item_tags`, `item_preview_url`, `item_content_descriptors`, `item_statistics`, `item_statistic`, `item_metadata`, `item_children`, `item_key_value_tags`, `item_key_value_tag`, `item_state`, `item_state_flags`, `item_state_subscribed`, `item_state_installed`, `item_state_needs_update`, `item_state_downloading`, `item_state_download_pending`, `item_download_info`, `item_download_info_available`, `item_downloaded_bytes`, `item_download_total_bytes`, `item_download_progress`, `item_download_complete`, `item_install_info`, `item_install_info_available`, `item_install_folder`, `item_size_on_disk`, `item_install_timestamp`, `download_item_results`, `download_item_result`, and `download_item_failed` without retaining message history. Deleting an item clears its item-level cached snapshots.
...
`SteamworksUgcCommand::init_workshop_for_game_server(AppId(480), "workshop_server")` initializes Workshop storage through a `SteamworksServer` resource instead of a `SteamworksClient` resource. Dedicated server apps get that command layer from `SteamworksServerPlugins` by default; apps with a custom lifecycle can add `SteamworksServerPlugin` and `SteamworksUgcPlugin` together before sending it.
...
`SteamworksRemotePlayState` caches the latest bulk session list, bounded ID-based known session snapshots, bounded callback-observed connected session IDs, the latest submitted Remote Play Together invite, and successful invite count. Systems can query bulk snapshots with `session_count`, `sessions_for_user`, and `latest_session_for_user`; query specific ID-based snapshots with `known_session`, `has_known_session`, and `known_sessions_for_user`; read the latest user session with `latest_known_session_for_user`; inspect fields through `session_user`, `session_client_name`, `session_client_form_factor`, and `session_client_resolution`; check callback-observed connectivity with `observed_connected_session_count` and `is_session_observed_connected`; and inspect the last invite through `last_submitted_invite`.
...
Timeline commands validate strings before calling upstream `steamworks`, so interior NUL bytes become `SteamworksTimelineError::InvalidString` instead of panicking in a C string conversion. Event start offsets must be finite. `SteamworksTimelineState` tracks the current game mode, state description, total submitted event count, latest event, and bounded event history through `events()`. Systems can read common UI fields directly with `has_state_description`, `state_description_text`, `state_description_duration`, `has_events`, `cached_event_count`, `events_with_icon`, `last_event_with_icon`, `events_with_clip_priority`, `last_event_with_clip_priority`, `last_event_icon`, `last_event_title`, `last_event_description`, `last_event_priority`, `last_event_start_offset_seconds`, `last_event_duration`, and `last_event_clip_priority` instead of unpacking event snapshots. The upstream Timeline wrapper no-ops when the Steam client API is too old for Timeline support; `SteamworksTimelineResult::Ok` means the command was accepted by this Bevy command layer and submitted to the wrapper.
...
`SteamworksAppsCommand::get_current_app_info()` combines the most commonly needed app checks into one `SteamworksCurrentAppInfo` snapshot. `SteamworksAppsState` also caches bounded individual app/DLC checks, install directories, language, beta branch, launch command line, and launch query parameter reads for systems that prefer resource lookups over retaining message history. Systems can query `current_app_id`, `current_app_owned`, `known_app_install_check_count`, `known_dlc_install_check_count`, `known_subscribed_app_check_count`, `available_game_language_count`, `supports_game_language`, `current_game_language_is`, `is_on_beta_branch`, `has_launch_command_line`, `launch_query_param_was_read`, and `launch_query_param_has_value` instead of unpacking snapshots for common app/ownership UI checks. Use `current_beta_name_result()` when you need to distinguish an unread beta branch from a completed read that reported no beta branch. Launch query keys are validated before calling upstream `steamworks`, so interior NUL bytes become `SteamworksAppsError::InvalidString` instead of panicking. If Steam delivers new URL launch parameters while the app is already running, `SteamworksEvent::NewUrlLaunchParameters` is also mirrored as `SteamworksAppsOperation::NewUrlLaunchParametersReceived`; send `get_launch_command_line()` or `get_launch_query_param(...)` afterwards to read the latest values.
...
Aggregated global stats use `request_global_stats(history_days)` first; after `GlobalStatsReceived`, read values with `get_global_stat_i64`, `get_global_stat_f64`, `get_global_stat_history_i64`, or `get_global_stat_history_f64`. `SteamworksStatsState` caches the latest game ID plus bounded value snapshots (`SteamworksGlobalStatValue<T>`) and history snapshots (`SteamworksGlobalStatHistory<T>`) by stat name, available through `global_stat_i64`, `global_stat_f64`, `global_stat_history_i64`, `global_stat_history_f64`, `global_stat_history_i64_series`, and `global_stat_history_f64_series`. A new global stats request clears the previous cached global stat values until fresh reads complete.

Leaderboard find and find-or-create commands are asynchronous. Successful results insert the upstream leaderboard handle into the plugin and return a `SteamworksLeaderboardId`; later info reads, score uploads, entry downloads, and forget commands use that stable ID. `SteamworksStatsState` caches bounded leaderboard snapshots for the latest find/create requests and completions, info reads, score upload requests and completions, entries download requests and completions, and the latest forgotten leaderboard ID. It also keeps bounded name-to-ID, metadata, upload, and entry lookups, available through `leaderboard_id`, `has_leaderboard_id`, `leaderboards`, `leaderboard_info`, `has_leaderboard_info`, `leaderboard_info_by_name`, `leaderboard_name`, `leaderboard_display_type`, `leaderboard_sort_method`, `leaderboard_entry_count`, `leaderboard_score_upload_request`, `leaderboard_score_upload_result`, `leaderboard_score_upload`, `leaderboard_uploaded_score`, `leaderboard_score_was_changed`, `leaderboard_uploaded_rank_new`, `leaderboard_uploaded_rank_previous`, `leaderboard_entries_download_request`, `leaderboard_entries_download_result`, `leaderboard_entries`, `leaderboard_downloaded_entry_count`, `last_leaderboard_downloaded_entry_count`, `leaderboard_entry_by_user`, `leaderboard_has_entry_for_user`, `leaderboard_entry_by_rank`, `leaderboard_has_rank`, `leaderboard_score_by_user`, `leaderboard_rank_by_user`, and `leaderboard_entry_details`. Forgetting a leaderboard clears its keyed metadata and result caches. Global downloads use absolute rank ranges, user-relative downloads accept signed offsets around the current user, and friends downloads do not take a range. Ranged downloads are capped per command to keep frame work bounded.
