...
| User stats | Implemented | Current stats lifecycle, int/float stats with bounded lookup caches, achievements, global stats/history, global achievement percentages, leaderboards with bounded ID/metadata caches, store/reset. |
...
| Input | Implemented | Init/run/shutdown, controller and action handles with bounded lookup caches, digital/analog data, origins/glyphs/names, cached origin presentation lookup, motion data, action set activation, binding panel. |
| Screenshots | Implemented | Hook state, trigger, library add with bounded submission caches, requested/ready callbacks. |
